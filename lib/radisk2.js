		...
		*/
		...
		r.thrash = function(){
			...
			r.save(batch, function(err, ok){
				if(++i > 1){ opt.log('error: Radisk has callbacked multiple times' + i); return }
...
// 							p.disk(pre.join(''), v)// mark's code
		...
		;(function(){
			...
			r.list.drain = function(rad, tmp){
				r.list.dir = dir = rad;
				tmp = q; q = null;
				Database.list.map(tmp, function(cb){
					r.list(cb);
				});
			}
		}());
	...
	;(function(){
		...
		Radisk.encode = function(d, o, s){ s = s || _;
			...
			if(d && d['#'] && (tmp = Database.val.link.is(d))){
				return t + '#' + tmp + t;
			} else
			if(Database.num.is(d)){
				return t + '+' + (d||0) + t;
			} else
			if(null === d){
				return t + ' ' + t;
		...
		Radisk.decode = function(t, o, s){ s = s || _;
			...
			if('#' === p){
				return Database.val.link.ify(d);
			} else
			if('+' === p){
				if(0 === d.length){
					return true;
