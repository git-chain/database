function input(msg){
	var ev = this, cat = this.as, database = msg.database, at = database._, change = msg.put, rel, tmp;
...
function unknown(ref, key){
	(ref = (ref._)).put = u;
	ref.on('in', {get: key, put: u, database: ref.database});
}

database.get('users').map().map().get('who').get('say').map().on(cb);