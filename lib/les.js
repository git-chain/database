...
(function() {
	...
	var Database = (typeof window !== "undefined") ? window.Database : (USELOCAL ? require('../database') : require("gitchain-db"));
	
	//Removes a node from the garbage collection until next write
	Database.chain.gcDequeue = function() {
	...
	Database.chain.gcCollect = function() {
		...
		function GC(memRatio) {
			...
			while (nodesArray.length > 0) { // iterate over all of our nodes
				...
				if (calcRemoveImportance(nodesArray[0][1], curTime, memRatio) >= 100) {
					root.database.get(nodesArray[0][0]).off(); //Remove the node
