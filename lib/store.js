    ...
    root.on('get', function(msg){
        ...
        dare(key||'', function(err, data, info){
            ...
            if(info.unit && data && u !== (va = data[':']) && u !== (ve = data['>'])){ // new format
                ...
                (graph = graph || {})[so] = Database.state.ify(graph[so], ha, ve, va, so);
        ...
        function each(val, has, a,b){ // TODO: THIS CODE NEEDS TO BE FASTER!!!!
            ...
            (graph = graph || {})[soul] = Database.state.ify(graph[soul], has, state, val, soul);
        }
    });
    var val_is = Database.valid;
