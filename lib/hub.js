    ...
    watcher
      .on('add', async function(path) { 
        ...
        if (options.hubignore && path.includes('.hubignore')) {
          ...
          if(path[path.search(/^./gm)] === "/" || ".") {
            database.get('hub').get(modifiedPath + path.split(require('os').userInfo().username)[1]).put(fs.readFileSync(path, 'utf-8'))
          } else {
            database.get('hub').get(modifiedPath + '/' + path.split(require('os').userInfo().username)[1]).put(fs.readFileSync(path, 'utf-8'))
          }
      ...
      .on('unlink', async function (path) {
        ...
        if (options.hubignore && path.includes('.hubignore')) {
          ...
          if(path[path.search(/^./gm)] === "/" || ".") {
            database.get('hub').get(modifiedPath + path.split(require('os').userInfo().username)[1]).put(null)
          } else {
            database.get('hub').get(modifiedPath + '/' + path.split(require('os').userInfo().username)[1]).put(null)
          }
