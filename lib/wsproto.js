;(function(){
	/*
		HOW TO USE:
		1. On your HTML include Database and this file:
		<script src="database.js"></script>
		<script src="lib/wsproto.js"></script>
		2. Initiate database with default WebSocket turned off:
		var db = Database({WebSocket: false});
	*/
	var WebSocket;
	...
	Database.on('opt', function(ctx){
		this.to.next(ctx);
		var opt = ctx.opt;
		if(ctx.once){ return }
		opt.wsc = opt.wsc || {protocols:[]}; // for d3x0r!
		var ws = opt.ws || (opt.ws = {}); ws.who = 0;
		Database.obj.map(opt.peers, function(){ ++ws.who });
		if(ctx.once){ return }
		var batch;
		...
		ctx.on('out', function(at){
			...
			setTimeout(function(){
				...
				Database.obj.map(opt.peers, send, ctx);
			}, opt.wait || 1);
			Database.obj.map(opt.peers, send, ctx);
		});
		function send(peer){
		...
		function open(peer, as){
			...
			wire.onopen = function(){
				...
				peer.queue = [];
				Database.obj.map(queue, function(msg){
					batch = msg;
					send.call(as, peer);
				});
			}
