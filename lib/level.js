...
var Database   = ('undefined' !== typeof window) ? window.Database : require('../database');
var debug = false;
...
Database.on('opt', function(ctx) {
  ...
  ctx.on('put', function(msg) {
   this.to.next(msg);
    ...
    opt.level.get(soul, function(err, data) {
      ...
      data = Database.state.ify(data, key, state, val, soul);
  ...
  ctx.on('get', function(msg) {
    ...
    opt.level.get(soul, function(err, data) {
      ...
      if (data && has) {
        data = Database.state.to(data, has);
      }

      // Emulate incoming ack
      ctx.on('in', {
        '@' : msg['#'],
        put : Database.graph.node(data),
      });
    });
  });
