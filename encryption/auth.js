    ...
    User.prototype.auth = function(...args){ // TODO: this PR with arguments need to be cleaned up / refactored.
      ...
      act.b = function(list){
        ...
        if(u === get){
          if(act.name){ return act.err('User account is not published for dApps to access') }
      ...
      act.c = function(auth){
        ...
        Encryption.work(pass, (act.auth = auth).s, act.d, act.enc); // the proof of work is evidence that we've spent some time/effort trying to log in, this slows brute force.
      }
      act.d = function(proof){
        Encryption.decrypt(act.auth.ek, proof, act.e, act.enc);
      }
      act.e = function(half){
        if(u === half){
      ...
      act.g = function(pair){
        ...
        at.encryption = act.pair;
        cat.ing = false;
        try{if(pass && u == (obj_ify(cat.root.graph['~'+pair.pub].auth)||'')[':']){ opt.shuffle = opt.change = pass; } }catch(e){} // migrate UTF8 & Shuffle!
        opt.change? act.z() : (cb || noop)(at);
        if(Encryption.window && ((database.back('user')._).opt||opt).remember){
      ...
      act.z = function(){
        ...
        Encryption.work(opt.change, act.salt, act.y);
      }
      act.y = function(proof){
        Encryption.encrypt({priv: act.pair.priv, epriv: act.pair.epriv}, proof, act.x, {raw:1});
      }
      act.x = function(auth){
      ...
      act.err = function(e){
        var ack = {err: Database.log(e || 'User cannot be found')};
        cat.ing = false;
        (cb || noop)(ack);
      }
