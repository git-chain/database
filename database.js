;(function(){
  ...
  /* UNBUILD */
	...
	;USE(function(module){
		...
		USE('./shim');
		Database.valid = USE('./valid');
		Database.state = USE('./state');
		Database.on = USE('./onto');
		Database.dup = USE('./dup');
		Database.ask = USE('./ask');
	...
	;USE(function(module){
		var Database = USE('./index'), next = Database.chain.get.next;
	...
	;USE(function(module){
		...
		json.sucks = function(d){ if(d > 99){ console.log("JSON blocking CPU detected"); json.sucks = noop } }
			...
			var hear = mesh.hear = function(raw, peer){
				...
				if('[' === tmp){
					parse(raw, function(err, msg){
						if(err || !msg){ return mesh.say({dam: '!', err: "JSON parse error"}, peer) }
...
//if(opt.super && (!ack || !msg.put)){ return } // TODO: MANHATTAN STUB //OBVIOUSLY BUG! But squelch relay. // :( get only is 100%+ CPU usage :(
			...
			function flush(peer){
				...
				}catch(e){return opt.log('JSON stringify error', e)}}
				if(!tmp){ return }
				send(tmp, peer);
			}
