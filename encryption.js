;(function(){
  ...
  ;USE(function(module){
    var Encryption = USE('./root');
    try{ if(Encryption.window){
  ...
  ;USE(function(module){
    ...
    if(u+''== typeof btoa){
      if(u+'' == typeof Buffer){
        try{ global.Buffer = USE("buffer", 1).Buffer }catch(e){ }
      }
  ...
  ;USE(function(module){
    ...
    Encryption.decrypt = Encryption.decrypt || (async (data, pair, cb, opt) => { try {
      opt = opt || {};
      var key = (pair||opt).epriv || pair;
      if(!key){
        if(!Encryption.I){ throw 'No decryption key.' }
        pair = await Encryption.I(null, {what: data, how: 'decrypt', why: opt.why});
        key = pair.epriv || pair;
      }
      var json = await S.parse(data);
      var buf, bufiv, bufct; try{
        ...
        if(Encryption.opt.fallback){
          opt.encode = 'utf8';
          return await Encryption.decrypt(data, pair, cb, opt);
        }
      }
  ...
  ;USE(function(module){
    ...
    var Encryption = USE('./root');
    Encryption.work = USE('./work');
    Encryption.sign = USE('./sign');
    Encryption.verify = USE('./verify');
    Encryption.encrypt = USE('./encrypt');
    Encryption.decrypt = USE('./decrypt');
    Encryption.certify = USE('./certify');

    Encryption.random = Encryption.random || shim.random;

    // For documentation see https://nodejs.org/api/buffer.html
    Encryption.Buffer = Encryption.Buffer || USE('./buffer');
  ...
  ;USE(function(module){
    var Encryption = USE('./encryption'), Database, u;
    if(Encryption.window){
      Database = Encryption.window.Database || {chain:{}};
    } else {
      Database = USE((u+'' == typeof MODULE?'.':'')+'./database', 1);
    }
  ...
  ;USE(function(module){
    var u, Database = (''+u != typeof window)? (window.Database||{chain:{}}) : USE((''+u === typeof MODULE?'.':'')+'./database', 1);
  ...
  ;USE(function(module){
    var User = USE('./user'), Encryption = User.Encryption, Database = User.Database, noop = function(){};
  ...
  ;USE(function(module){
    var User = USE('./user'), Encryption = User.Encryption, Database = User.Database;
  ...
  ;USE(function(module){
    var Encryption = USE('./encryption'), S = USE('./settings'), noop = function() {}, u;
    var Database = (''+u != typeof window)? (window.Database||{on:noop}) : USE((''+u === typeof MODULE?'.':'')+'./database', 1);

    Database.on('opt', function(at){
      if(!at.encryption){
        at.encryption = {own: {}};
        at.on('put', check, at);
      }
    ...
    function check(msg){ // REVISE / IMPROVE, NO NEED TO PASS MSG/EVE EACH SUB?
      ...
      if((msg._||'').faith && (at.opt||'').faith && 'function' == typeof msg._){
        Encryption.opt.pack(put, function(raw){
        Encryption.verify(raw, false, function(data){ // this is synchronous if false
          put['='] = Encryption.opt.unpack(data);
          eve.to.next(msg);
        })})
        return 
      }
      ...
      if(0 <= soul.indexOf('<?')){ // special case for "do not sync data X old" forget
        ...
        if(tmp && (state < (Database.state() - (tmp * 1000)))){ // sec to ms
      ...
      if(tmp = Encryption.opt.pub(soul)){ // special case, account data for a public key.
    ...
    check.hash = function(eve, msg, val, key, soul, at, no){
      Encryption.work(val, null, function(data){
    ...
    check.pub = async function(eve, msg, val, key, soul, at, no, user, pub){ var tmp // Example: {_:#~asdf, hello:'world'~fdsa}}
      ...
      const verify = (certificate, certificant, cb) => {
        if (certificate.m && certificate.s && certificant && pub)
          ...
          return Encryption.verify(certificate, pub, data => { // check if "pub" (of the graph owner) really issued this cert
            ...
            if (u !== data && data.c && data.w && (data.c === certificant || data.c.indexOf('*' || certificant) > -1)) {
              ...
              String.match = String.match || Database.text.match
      ...
      if ((tmp = user.is) && tmp.pub && !raw['*'] && !raw['+'] && (pub === tmp.pub || (pub !== tmp.pub && ((msg._.msg || {}).opt || {}).cert))){
        Encryption.opt.pack(msg.put, packed => {
          Encryption.sign(packed, (user._).encryption, async function(data) {
            if (u === data) return no(Encryption.err || 'Signature fail')
            msg.put[':'] = {':': tmp = Encryption.opt.unpack(data.m), '~': data.s}
            msg.put['='] = tmp
  
            // if writing to own graph, just allow it
            if (pub === user.is.pub) {
              if (tmp = link_is(val)) (at.encryption.own[tmp] = at.encryption.own[tmp] || {})[pub] = 1
      ...
      Encryption.opt.pack(msg.put, packed => {
        Encryption.verify(packed, raw['*'] || pub, function(data){ var tmp;
          data = Encryption.opt.unpack(data);
          if (u === data) return no("Unverified data") // make sure the signature matches the account it claims to be on. // reject any updates that are signed with a mismatched account.
          if ((tmp = link_is(data)) && pub === Encryption.opt.pub(tmp)) (at.encryption.own[tmp] = at.encryption.own[tmp] || {})[pub] = 1
    ...
    var valid = Database.valid, link_is = function(d,l){ return 'string' == typeof (l = valid(d)) && l }, state_ify = (Database.state||'').ify;

    var pubcut = /[^\w_-]/; // anything not alphanumeric or _ -
    Encryption.opt.pub = function(s){
      if(!s){ return }
      s = s.split('~');
      if(!s || !(s = s[1])){ return }
    ...
    Encryption.opt.stringy = function(t){
      // TODO: encrypt etc. need to check string primitive. Make as breaking change.
    }
    Encryption.opt.pack = function(d,cb,k, n,s){ var tmp, f; // pack for verifying
      if(Encryption.opt.check(d)){ return cb(d) }
      if(d && d['#'] && d['.'] && d['>']){ tmp = d[':']; f = 1 }
      JSON.parseAsync(f? tmp : d, function(err, meta){
        var sig = ((u !== (meta||'')[':']) && (meta||'')['~']); // or just ~ check?
        if(!sig){ cb(d); return }
        cb({m: {'#':s||d['#'],'.':k||d['.'],':':(meta||'')[':'],'>':d['>']||Database.state.is(n, k)}, s: sig});
      });
    }
    var O = Encryption.opt;
    Encryption.opt.unpack = function(d, k, n){ var tmp;
      if(u === d){ return }
      if(d && (u !== (tmp = d[':']))){ return tmp }
      k = k || O.fall_key; if(!n && O.fall_val){ n = {}; n[k] = O.fall_val }
      if(!k || !n){ return }
      if(d === n[k]){ return d }
      if(!Encryption.opt.check(n[k])){ return d }
      var soul = (n && n._ && n._['#']) || O.fall_soul, s = Database.state.is(n, k) || O.fall_state;
      if(d && 4 === d.length && soul === d[0] && k === d[1] && fl(s) === fl(d[3])){
        return d[2];
      }
      if(s < Encryption.opt.shuffle_attack){
        return d;
      }
    }
    Encryption.opt.shuffle_attack = 1546329600000; // Jan 1, 2019
